...
@section('title', __('Daftar Reservasi'))
...
<div class="w-full flex flex-col Box-border">
    ...
    <div class="mb-6 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
            <h1 class="text-2xl font-bold text-[#2F3951] dark:text-slate-100 tracking-tight">{{ __('Reservasi Buku') }}</h1>
            <p class="text-gray-500 dark:text-slate-400 text-sm mt-1 font-medium">{{ __('Daftar reservasi buku Anda ketika stok sedang kosong.') }}</p>
        </div>
    </div>
    ...
    <div class="w-full bg-white dark:bg-slate-800 rounded-3xl border border-gray-200 dark:border-slate-700/50 shadow p-6 transition-colors duration-200">
        @if($reservasis->count() > 0)
            <div class="overflow-x-auto w-full">
                <table class="w-full text-sm text-left text-gray-500 dark:text-slate-350 border-collapse">
                    <thead>
                        <tr class="border-b border-gray-100 dark:border-slate-700/50 text-[10px] font-bold text-gray-400 dark:text-slate-400 uppercase tracking-wider">
                            ...
                            <th scope="col" class="pb-4 font-bold">{{ __('Tanggal Reservasi') }}</th>
                        ...
                        @foreach($reservasis as $res)
                            <tr>
                                ...
                                <td class="py-4 text-center">
                                    @if($res->status === 'menunggu')
                                        <form action="{{ route('member.reservasi.cancel', $res->id) }}" method="POST" onsubmit="return confirm('{{ __('Apakah Anda yakin ingin membatalkan reservasi buku ini?') }}')" class="m-0">
                                            @csrf
        ...
        @else
            ...
            <div class="flex flex-col items-center justify-center py-16 text-center text-gray-400 dark:text-slate-500">
                ...
                <h3 class="text-sm font-bold text-[#2F3951] dark:text-slate-200 mb-1">{{ __('Tidak ada reservasi aktif') }}</h3>
                <p class="text-xs text-gray-400 dark:text-slate-500 max-w-xs leading-relaxed">{{ __('Reservasi buku hanya bisa dilakukan untuk buku yang stoknya sedang habis.') }}</p>
