                    ...
                    <tbody class="divide-y divide-gray-50 dark:divide-slate-800 text-sm text-[#2F3951] dark:text-slate-300">
                        @forelse($overdueLoans as $loan)
                            ...
                            <tr class="hover:bg-gray-50/50 dark:hover:bg-slate-800/30 transition-colors">
                                ...
                                {{-- AKSI --}}
                                <td class="px-6 py-4 text-center">
                                    <form action="{{ route(auth()->user()->role . '.member.peminjaman.kembali', $loan->id) }}" method="POST" onsubmit="return confirm('Apakah buku ini akan dikembalikan sekarang? Denda otomatis akan dibuat.')">
                                        @csrf
