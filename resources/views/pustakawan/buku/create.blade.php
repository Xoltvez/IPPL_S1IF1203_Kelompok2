...
<div class="w-full flex flex-col text-left md:px-2 max-w mx-auto Box-border">
    ...
    <div class="w-full bg-white rounded-2xl border border-gray-200 shadow overflow-hidden">
        ...
        <form action="{{ route(auth()->user()->role . '.buku.store') }}" method="POST" enctype="multipart/form-data" class="p-6 md:p-8 w-full m-0 flex flex-col gap-6">
            @csrf

        <div class="w-full flex flex-col text-left" x-data="{ imgPreview: null }">
                ...
                <div class="flex-1 w-full flex flex-col justify-center min-h-[160px]">
                    ...
                    <input
                        ...
                        @change="
                            ...
                            if (file) { imgPreview = URL.createObjectURL(file); }
                        "
                    ...
                    <p class="text-[10px] text-gray-400 mt-2 leading-relaxed">
                        ...
                        Gambar akan otomatis dipotong mengikuti proporsi standar katalog MacaBae.
                    </p>
            ...
            <div class="w-full flex flex-col text-left">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-5 w-full">
                    ...
                    <div class="flex flex-col text-left w-full">
                        ...
                        <input type="number" name="tahun_terbit" id="tahun_terbit" value="{{ old('tahun_terbit') }}" placeholder="Contoh: 2015" style="background-color: #FCFCFC" class="w-full px-4 py-2.5 border border-gray-200 focus:border-[#4D9BE2]/50 focus:bg-white rounded-xl text-sm text-[#2F3951] placeholder-gray-400 focus:outline-none focus:ring-4 focus:ring-[#4D9BE2]/5 transition-all">
                    ...
                    <div class="flex flex-col text-left w-full">
                        ...
                        <div class="relative w-full">
                            <select name="kategori_id" id="kategori_id" required style="background-color: #FCFCFC" class="w-full px-4 py-2.5 border border-gray-200 focus:border-[#4D9BE2]/50 focus:bg-white rounded-xl text-sm text-[#2F3951] focus:outline-none focus:ring-4 focus:ring-[#4D9BE2]/5 transition-all appearance-none cursor-pointer">
                                <option value="" disabled {{ old('kategori_id', $selectedKategoriId ?? '') == '' ? 'selected' : '' }}>-- Pilih Kategori Buku --</option>
                                @foreach($kategoris as $kategori)
                                    <option value="{{ $kategori->id }}" data-is-ebook="{{ strtolower($kategori->nama_kategori) === 'e-book' ? '1' : '0' }}" {{ old('kategori_id', $selectedKategoriId ?? '') == $kategori->id ? 'selected' : '' }}>
            ...
            <div class="mt-4 w-full flex flex-col text-left">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-5 w-full">
                    <div class="flex flex-col text-left w-full md:col-span-2">
                        ...
                        <textarea name="deskripsi" id="deskripsi" rows="3" placeholder="Masukkan deskripsi lengkap buku..." class="w-full px-4 py-2.5 border border-gray-200 focus:border-[#4D9BE2]/50 focus:bg-white rounded-xl text-sm text-[#2F3951] placeholder-gray-400 focus:outline-none focus:ring-4 focus:ring-[#4D9BE2]/5 transition-all">{{ old('deskripsi') }}</textarea>
                    ...
                    <div class="flex flex-col text-left w-full md:col-span-2">
                        ...
                        <textarea name="sinopsis" id="sinopsis" rows="3" placeholder="Masukkan sinopsis atau ringkasan cerita buku..." class="w-full px-4 py-2.5 border border-gray-200 focus:border-[#4D9BE2]/50 focus:bg-white rounded-xl text-sm text-[#2F3951] placeholder-gray-400 focus:outline-none focus:ring-4 focus:ring-[#4D9BE2]/5 transition-all">{{ old('sinopsis') }}</textarea>
                ...
                <div class="grid grid-cols-1 gap-5 w-full">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-5 w-full">
                        ...
                        <div class="flex flex-col text-left w-full">
                            ...
                            <input type="number" name="halaman" id="halaman" value="{{ old('halaman') }}" min="1" max="100" placeholder="Contoh: 10" style="background-color: #FCFCFC" class="w-full px-4 py-2.5 border border-gray-200 focus:border-[#4D9BE2]/50 focus:bg-white rounded-xl text-sm text-[#2F3951] placeholder-gray-400 focus:outline-none focus:ring-4 focus:ring-[#4D9BE2]/5 transition-all">
            ...
            <div class="mt-4 w-full flex flex-col text-left">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-5 w-full">
                    <div class="flex flex-col text-left w-full" id="stok-wrapper">
                        ...
                        <input type="number" name="stok" id="stok" value="{{ old('stok') }}" required placeholder="Contoh: 10" style="background-color: #FCFCFC" class="w-full px-4 py-2.5 border border-gray-200 focus:border-[#4D9BE2]/50 focus:bg-white rounded-xl text-sm text-[#2F3951] placeholder-gray-400 focus:outline-none focus:ring-4 focus:ring-[#4D9BE2]/5 transition-all">
                    ...
                    <div class="flex flex-col text-left w-full">
                        ...
                        <div class="relative w-full">
                            <select name="status" id="status" required style="background-color: #FCFCFC" class="w-full px-4 py-2.5 border border-gray-200 focus:border-[#4D9BE2]/50 focus:bg-white rounded-xl text-sm text-[#2F3951] focus:outline-none focus:ring-4 focus:ring-[#4D9BE2]/5 transition-all appearance-none cursor-pointer">
                                <option value="aktif" {{ old('status') == 'aktif' ? 'selected' : '' }}>Aktif (Muncul di Member)</option>
                                <option value="non aktif" {{ old('status') == 'non aktif' ? 'selected' : '' }}>Non Aktif (Sembunyikan dari Member)</option>
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const oldEbookContents = @json(old('ebook_contents') ?? []);
        const oldHalaman = @json(old('halaman'));
        const oldEbookChapters = @json(old('ebook_chapters') ?? []);
        ...
        // Load old chapters if validation fails
        if (oldEbookChapters && Object.keys(oldEbookChapters).length > 0) {
            chaptersContainer.innerHTML = '';
            Object.values(oldEbookChapters).forEach(ch => {
                if (ch.title && ch.start_page) {
                    addChapterRow(ch.title, ch.start_page);
                }
            });
        }
