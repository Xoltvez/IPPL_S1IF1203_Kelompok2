...
<div class="w-full flex flex-col Box-border">
    ...
    <div class="mb-4 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
            <h1 class="text-2xl font-bold text-[#2F3951] tracking-tight">Buku Tersimpan</h1>
            <p class="text-gray-500 text-sm mt-1 font-medium">Koleksi buku favorit yang telah Anda tandai.</p>
        </div>
    </div>
    ...
    <div class="w-full flex flex-col md:flex-row items-stretch md:items-center justify-between gap-4 mb-6">
        ...
        <div class="flex items-center gap-4 flex-wrap md:flex-nowrap">
            <!-- Count Pill Card -->
            <div class="bg-white border border-gray-200 rounded-2xl px-4 py-2 flex items-center gap-3 shadow-sm flex-shrink-0 min-w-[120px]">
                ...
                <div class="leading-tight text-left">
                    <p class="text-[9px] font-bold text-gray-400 uppercase tracking-wider">Buku tersimpan</p>
                    <p class="text-sm font-black text-[#2F3951]">{{ $bukuFavorit->total() }}</p>
                </div>
            </div>

            <!-- Hapus Semua Action Button -->
            @if($bukuFavorit->total() > 0)
                <form action="{{ route('member.tersimpan.clear') }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus semua buku dari daftar tersimpan?')" class="m-0 flex-shrink-0">
                    @csrf
                    @method('DELETE')
        ...
        <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-6">
            @foreach($bukuFavorit as $buku)
                <div class="relative flex flex-col">
                    <div class="relative block w-full aspect-[3/4] rounded-2xl overflow-hidden shadow-sm border border-gray-100 bg-[#FCFCFC] group">
                        
                        <!-- Hover Overlay with Title, Author & Quick Remove (Premium Polish) -->
                        <div class="absolute inset-0 bg-gradient-to-t from-[#2F3951]/95 via-[#2F3951]/50 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 z-10 flex flex-col justify-end p-4 text-left">
    ...
    @else
        ...
        <div class="flex flex-col items-center justify-center py-20 text-center text-gray-400">
            ...
            <h3 class="text-sm font-bold text-[#2F3951] mb-1">Belum ada buku tersimpan</h3>
            <p class="text-xs text-gray-400 max-w-xs leading-relaxed">Pilih kategori atau gunakan pencarian untuk mencari buku favorit Anda.</p>
